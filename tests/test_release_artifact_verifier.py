...
def _metadata(name="zebbern-kali-mcp", version="1.0.1"):
    return "\n".join(
        [
...
def _write_valid_dist(
    ...
    *,
    ...
    wheel_name = "zebbern_kali_mcp-1.0.1-py3-none-any.whl"
    sdist_name = "zebbern_kali_mcp-1.0.1.tar.gz"
    with zipfile.ZipFile(dist / wheel_name, "w") as archive:
        for name, data in source_files.items():
            if name in {"README.md", "pyproject.toml", "LICENSE"}:
                continue
            archive.writestr(name, data)
        archive.writestr("zebbern_kali_mcp-1.0.1.dist-info/licenses/LICENSE", source_files["LICENSE"])
        archive.writestr("zebbern_kali_mcp-1.0.1.dist-info/METADATA", metadata)
        archive.writestr("zebbern_kali_mcp-1.0.1.dist-info/WHEEL", b"Wheel-Version: 1.0\n")
        archive.writestr(
            "zebbern_kali_mcp-1.0.1.dist-info/entry_points.txt",
            b"[console_scripts]\nzebbern-kali-mcp = mcp_server:main\n",
        )
        archive.writestr("zebbern_kali_mcp-1.0.1.dist-info/top_level.txt", b"mcp_server\nmcp_tools\n")
        archive.writestr("zebbern_kali_mcp-1.0.1.dist-info/RECORD", b"")
        for name, data in (extra_wheel_members or {}).items():
            archive.writestr(name, data)
    with TarFile.open(dist / sdist_name, "w:gz") as archive:
        prefix = "zebbern-kali-mcp-1.0.1/"
        sdist_members = {
            **source_files,
            "PKG-INFO": metadata,
...
def test_valid_wheel_and_sdist_produce_deterministic_manifest(tmp_path):
    ...
    assert list(manifest) == [
        "zebbern_kali_mcp-1.0.1-py3-none-any.whl",
        "zebbern_kali_mcp-1.0.1.tar.gz",
    ]
...
def test_corrupted_source_file_is_rejected(tmp_path):
    dist = _write_valid_dist(tmp_path)
    wheel = dist / "zebbern_kali_mcp-1.0.1-py3-none-any.whl"
