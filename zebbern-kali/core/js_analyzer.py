...
class JSAnalyzer:
    ...
    def full_scan(self, url: str, depth: int = 2) -> Dict[str, Any]:
        """
        Complete JS security scan: discover all JS files and analyze each.
        ...
        try:
            ...
            discovery = self.discover_js_files(url, depth=depth)
            
            if not discovery.get("success"):
                return discovery
            
            js_files = discovery["js_files"]["internal"] + discovery["js_files"]["external"]
            ...
            # Step 2: Analyze all JS files
            analysis = self.analyze_multiple(js_files[:50])  # Limit to 50 files
            
            # Step 3: Generate summary
            high_risk_secrets = [
                s for s in analysis.get("results", [])
            ...
            report_file = os.path.join(
                self.output_dir, "reports",
                f"fullscan_{parsed.netloc}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            ...
            full_results = {
                ...
                "summary": {
                    ...
                    "js_files_analyzed": analysis.get("files_analyzed", 0),
                    "total_secrets": analysis.get("total_secrets", 0),
                    "max_risk_level": analysis.get("max_risk_level", "NONE"),
                    "high_risk_files": len(high_risk_secrets)
                },
                "timestamp": datetime.now().isoformat()
            }
            ...
            full_results["report_file"] = report_file
