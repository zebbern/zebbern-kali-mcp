...
class APISecurityTester:
    ...
    def _ensure_dirs(self):
        ...
        os.makedirs(os.path.join(self.output_dir, "apifuzzer"), exist_ok=True)
    ...
    # ==================== APIFuzzer OpenAPI/Swagger Testing ====================
    
    def apifuzzer_scan(self, spec_url: str, target_url: str = "",
                       auth_header: str = "", test_level: int = 1,
                       additional_args: str = "") -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            test_level: Fuzz test level (1-5)
        ...
        try:
            report_dir = os.path.join(
                self.output_dir, "apifuzzer",
                f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            os.makedirs(report_dir, exist_ok=True)
            
            # Check if APIFuzzer is installed
            try:
                subprocess.run(["APIFuzzer", "--help"], capture_output=True, timeout=5)
            except FileNotFoundError:
                return {
                    "success": False,
                    "error": "APIFuzzer not found",
                    "install_command": "pip install APIFuzzer"
                }
            ...
            return {
                ...
                "findings": findings[:50] if isinstance(findings, list) else findings,
            ...
            return {"success": False, "error": "APIFuzzer timed out after 15 minutes"}
        except Exception as e:
            logger.error(f"APIFuzzer error: {e}")
            return {"success": False, "error": str(e)}
    ...
    def full_api_scan(self, target: str, openapi_spec: str = "",
                      wordlist: str = "", auth_header: str = "") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            arjun_result = self.arjun_discover(target, headers=headers)
            results["scans"]["arjun"] = arjun_result
            
            # 2. FFUF endpoint fuzzing
            logger.info("Running FFUF endpoint fuzzing...")
            fuzz_url = target.rstrip('/') + '/FUZZ'
            ffuf_result = self.ffuf_fuzz(
                fuzz_url,
                wordlist=wordlist or "/usr/share/wordlists/dirb/common.txt",
                headers=headers
            )
            results["scans"]["ffuf"] = ffuf_result
            
            # 3. Nuclei API scan
            logger.info("Running Nuclei API scan...")
            nuclei_result = self.nuclei_api_scan(target, tags="api")
            results["scans"]["nuclei"] = nuclei_result
            
            # 4. Kiterunner if available
            logger.info("Running Kiterunner API path discovery...")
            kr_result = self.kiterunner_scan(target)
            results["scans"]["kiterunner"] = kr_result
            
            # 5. OpenAPI/Swagger fuzzing if spec provided
            if openapi_spec:
                logger.info("Running APIFuzzer with OpenAPI spec...")
                apifuzzer_result = self.apifuzzer_scan(
                    openapi_spec, target_url=target,
                    auth_header=auth_header
                )
                results["scans"]["apifuzzer"] = apifuzzer_result
            
            # 6. Rate limit test
            logger.info("Running rate limit test...")
            rate_result = self.rate_limit_test(target, headers=headers)
            results["scans"]["rate_limit"] = rate_result
            
            # Generate summary
            summary = {
                "endpoints_found": (
                    ffuf_result.get("total_found", 0) +
                    kr_result.get("total_found", 0)
                ),
                "parameters_found": arjun_result.get("parameters_found", 0),
                "nuclei_findings": nuclei_result.get("total_findings", 0),
                "has_rate_limiting": rate_result.get("rate_limited", False)
            }
            ...
            report_file = os.path.join(
                ...
                f"full_api_scan_{urlparse(target).netloc}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            with open(report_file, 'w') as f:
            ...
            logger.error(f"Full API scan error: {e}")
            return {"success": False, "error": str(e)}
